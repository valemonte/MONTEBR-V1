--[[ Monte HUB | Moderate obfuscation (optimized for GitHub loader) ]]
-- orig:356082 bytes | renamed:375 locals
...
do
    local _v13 = _G.OxideStealAnEgg
    if _v13 and type(_v13.Unload) == "function" then pcall(_v13.Unload) end
end
local HUB = { conns = {}, drawings = {}, highlights = {}, dead = false }
_G.OxideStealAnEgg = HUB
local function _a12(conn) table.insert(HUB.conns, conn); return conn end
local function _v3(d) if d then table.insert(HUB.drawings, d) end; return d end
...
local _h6 = type(Library.SaveConfig) == "function"
    and type(Library.LoadConfig) == "function"
    and type(Library.ListConfigs) == "function"
local _t4 = "stealanegg"
local _a3 = {}
local function _s2(handle, applyFn)
    if handle and applyFn then
        table.insert(_a3, function() applyFn(handle:Get()) end)
    end
end
local function _k7()
    for _, fn in ipairs(_a3) do pcall(fn) end
end
local Players             = game:GetService("Players")
...
local _q4         = game:GetService("VirtualUser")
local LP          = Players.LocalPlayer
...
pcall(function()
    ...
    _a12(pps.PromptButtonHoldBegan:Connect(function(_m10, player)
        if player == LP and tostring(_m10) == "CarryAreaEgg" then
            _m10.HoldDuration = 0
        end
    end))
end)
pcall(function()
    local _y8 = game:GetService("CoreGui")
    _a12(_y8.ChildAdded:Connect(function(child)
        if child.Name == "PurchasePrompt" then
            task.wait(0.04)
            pcall(function()
                local _i10 = child:FindFirstChild("CancelButton", true)
                if _i10 and typeof(_i10) == "Instance" and _i10:IsA("GuiButton") then
                    pcall(function() _i10.MouseButton1Click:Fire() end)
                end
            end)
        end
    end))
end)
local function Notify(title, content, _w13, dur)
    pcall(function()
        Window:Notify({ Title = title, Content = content, Type = _w13 or "Info", Duration = dur or 2.5 })
    end)
end
local function _w3(fn)
    return function(...)
...
local function _g()
    if typeof(filtergc) ~= "function" or typeof(debug) ~= "table" or typeof(debug._w4) ~= "function" then
        return false, "no filtergc"
    end
    local ok, fn = pcall(function()
        return filtergc("function", {
            _n6 = { "gmatch", "GetFullName" },
        }, true)
    end)
    if not ok or type(fn) ~= "function" then
        return false, "filter miss"
    end
    local _c9 = (typeof(setrawmetatable) == "function" and setrawmetatable)
        or (typeof(setmetatable) == "function" and setmetatable)
    if not _c9 then
        return false, "no setmeta"
    end
    local _r9 = 0
    local _f13, ups = pcall(debug._w4, fn)
    if not _f13 or type(ups) ~= "table" then
        return false, "no upvalues"
    end
    for _, tbl in pairs(ups) do
        if typeof(tbl) == "table" then
            local _g12 = pcall(_c9, tbl, {
...
pcall(_g)
local function _b6(_i14, perChunk)
    local _k14 = getgc or (debug and debug.getgc)
    if type(_k14) ~= "function" then return end
    local ok, _e10 = pcall(_k14, true)
    if not ok or type(_e10) ~= "table" then return end
    perChunk = perChunk or 400
    for i = 1, #_e10 do
        local obj = _e10[i]
        _e10[i] = nil
        local _j11, _a13 = pcall(_i14, obj)
        if _j11 and _a13 == true then return end
        if i % perChunk == 0 then task.wait() end
    end
end
local _o8 = {}
do
    function _o8.FreezeTables()
        local _f10 = setrawmetatable or setmetatable
        local _n9 = getrawmetatable or getmetatable
        if not _f10 then return end
        _b6(function(obj)
            if typeof(obj) ~= "table" or (_n9 and _n9(obj)) then return end
            local _d9 = false
            ...
            if not _d9 then return end
            for _, v in pairs(obj) do
                if typeof(v) == "number" and v >= 1 and v <= 3 and obj[v] == nil then
                    pcall(_f10, obj, { __newindex = function() end })
                    break
                end
            end
        end)
    end
    function _o8.WipeUGI()
        local _x3 = _x3 or (debug and debug._x3)
        local _j4 = _j4 or (debug and debug._j4)
        local _i6 = _i6 or function(Function)
            return not pcall(setfenv, getfenv(Function))
        end
        if not (_x3 and _j4 and debug and debug.info) then return end
        _b6(function(Function)
            if typeof(Function) ~= "function" or not _i6(Function) then return end
            local ok, _p10 = pcall(debug.info, Function, "s")
            if not ok or type(_p10) ~= "string" then return end
            if not _p10:find("ReplicatedFirst", 1, true) or not _p10:find("UGI", 1, true) then return end
            local okC, _n6 = pcall(_x3, Function)
            if not okC or type(_n6) ~= "table" then return end
            for Index, Constant in next, _n6 do
                if type(Constant) == "string" and Constant == "Humanoid" then
                    pcall(_j4, Function, Index, "")
                end
            end
        end)
    end
    function _o8.ScrubX14()
        local _x3 = _x3 or (debug and debug._x3)
        local _i6 = _i6 or function(fn) return not pcall(setfenv, getfenv(fn)) end
        local _l11 = hookfunction or replaceclosure or hookfunc
        if not (_x3 and _l11 and debug and debug.getstack and debug.setstack) then return end
        _b6(function(fn)
            if typeof(fn) ~= "function" or not _i6(fn) then return end
            local ok, _k10 = pcall(_x3, fn)
            if not ok or type(_k10) ~= "table" or not table.find(_k10, "X-14") then return end
            local cb = nil
            pcall(function()
                cb = _l11(fn, function(...)
                    local _d12 = debug.getstack(1)
                    if type(_d12) == "table" then
                        for idx, val in pairs(_d12) do
                            if val == "X-14" then
    ...
    function _o8.SanitizeState()
        local _i6 = _i6 or function(v) return not pcall(setfenv, getfenv(v)) end
        local _w4 = _w4 or (debug and debug._w4)
        local _d6 = _d6 or (debug and debug._d6)
        local _j6 = _j6 or (debug and debug._j6)
        local _j3 = _j3 or function(f) return function(...) return f(...) end end
        if not (_w4 and _d6 and _j6) then return end
        _b6(function(v)
            if typeof(v) ~= "function" or not _i6(v) then return end
            local ok, _n13 = pcall(_w4, v)
            if not ok or type(_n13) ~= "table" or #_n13 ~= 19 then return end
            local ok2, u2 = pcall(_d6, v, 2)
            if not ok2 or typeof(u2) ~= "function" then return end
            local old = _j3(u2)
            pcall(_j6, v, 2, function(a, b)
...
task.spawn(function()
    pcall(_o8.FreezeTables)
    task.wait()
    pcall(_o8.WipeUGI)
    task.wait()
    pcall(_o8.ScrubX14)
    task.wait()
    pcall(_o8.SanitizeState)
end)
local function _q8() return LP.Character end
local function _e9()
...
local _j14 = bit32._j14
local unpack = table.unpack
local function _z10(n)
...
local function _v4()
    for _, s in ipairs(game:GetChildren()) do
        local ok, _b14 = pcall(s.GetDescendants, s)
        if ok and _b14 then
            for _, o in ipairs(_b14) do
                if o:IsA("RemoteEvent") and _z10(o.Name) then
                    _l6[o] = true
                    _x6 = _x6 or o
                end
            end
...
_v4()
local function _u3(v)
    if type(v) ~= "string" then return end
    local n = v:match("^X%-(%d+)$")
    return n and tonumber(n)
end
local function _x2(t, r)
...
local function _b7(r)
    for l=2,24 do
        local _, fn = pcall(debug.info, l, "f")
        if type(fn) == "function" then
            local _, ups = pcall(debug._w4, fn)
            if type(ups) == "table" then
                for _, v in pairs(ups) do
                    if _x2(v, r) then return v end
                    if type(v) == "table" then
                        local _w10
                        pcall(function()
                            for _, x in pairs(v) do
                                if _x2(x, r) then _w10 = x; return end
                            end
                        end)
                        if _w10 then return _w10 end
                    end
                end
            end
        end
...
local _o12 = nil
local function _d11(n)
    n = n % 1000
    return math.floor(n/100), math.floor(n/10)%10, n%10
end
local function _q10(m, c)
    local d1, d2, d3 = _d11(c)
    return m.prefix .. string._b13(_j14(d1, m.k1), _j14(d2, m.k2), _j14(d3, m.k3))
end
local function _z12(r, a1, a2)
    local st = _b7(r)
    if not st then return end
    local map = _l8(st, a1, a2)
    if not map.marker then return end
    local c = _u3(rawget(st, map.marker))
    if not c then return end
    local d1, d2, d3 = _d11(c)
    local m = {
        _w11 = st, map = map, remote = r,
        prefix = a1:sub(1, 9),
        k1 = _j14(a1:byte(10), d1),
        k2 = _j14(a1:byte(11), d2),
        k3 = _j14(a1:byte(12), d3),
        _f11 = c - os.time(),
        arg2 = a2
    }
    if _q10(m, c) == a1 then return m end
end
local function _s4(m)
    if m._w11 and m.map.marker then
        local _, raw = pcall(rawget, m._w11, m.map.marker)
        local c = _u3(raw)
        if c and math.abs((c - os.time()) - m._f11) <= 5 then
            return c
        end
    end
    return os.time() + m._f11
end
local function _i5(m)
    if m._w11 and m.map.arg2 then
        local _, v = pcall(rawget, m._w11, m.map.arg2)
        if type(v) == "string" then m.arg2 = v end
    end
    return m.arg2
end
local _l11 = hookfunction or replaceclosure or hookfunc or detour_function
if _x6 and _l11 then
    local _q9
    _q9 = _l11(_x6.FireServer, function(self, ...)
        local _z13 = table.pack(...)
        if not _l6[self] then
            return _q9(self, unpack(_z13, 1, _z13.n))
        end
        local a1 = _z13[1]
        if type(a1) == "string" and #a1 == 12 then
            if not _o12 then
                _o12 = _z12(self, a1, _z13[2])
            else
                local c = _u3(rawget(_o12._w11, _o12.map.marker))
                if c and _q10(_o12, c) ~= a1 then
                    local m = _z12(self, a1, _z13[2])
                    if m then m.spoofed = _o12.spoofed; _o12 = m end
                end
            end
            return _q9(self, unpack(_z13, 1, _z13.n))
        end
        if _o12 and type(a1) == "string" and #a1 == 4 then
            local c = _s4(_o12)
            _z13[1] = _q10(_o12, c)
            _z13[2] = _i5(_o12)
            _o12.spoofed = (_o12.spoofed or 0) + 1
            return _q9(self, unpack(_z13, 1, math.max(_z13.n, 2)))
        end
        return _q9(self, unpack(_z13, 1, _z13.n))
    end)
end
task.spawn(function()
    while not HUB.dead do
        task.wait(10)
        local _e12 = false
        for r in pairs(_l6) do
            if r:IsDescendantOf(game) then _e12 = true; break end
        end
        if not _e12 then
            table.clear(_l6)
            _x6 = nil
...
task.spawn(function()
    ...
    _a12(LP.CharacterAdded:Connect(function()
        task.wait(1)
        st = _c1()
    end))
    while not HUB.dead do
        if not st then
            ...
            if not st then
                _k11 = _k11 + 1
                local _b9 = math.min(5 * (2 ^ math.min(_k11 - 1, 3)), 30)
                local _v12 = 0
                while _v12 < _b9 and not HUB.dead do
                    task.wait(0.5)
                    _v12 = _v12 + 0.5
                end
            elseif _k11 > 0 then
...
local _g8, _j7, _e7, _w5, _y5, _p2, _t
pcall(function() _g8 = require(RS.Client._g8) end)
pcall(function() _j7 = require(RS.Client._j7) end)
pcall(function() _e7 = require(RS.Data.Areas) end)
pcall(function() _w5 = require(RS.Data.Rarity) end)
pcall(function() _y5 = require(RS.Data.Assets) end)
local _o5
pcall(function() _o5 = require(RS.Shared.Save) end)
pcall(function() _p2 = require(RS.Shared.Eggs._p2) end)
pcall(function()
    _t = (RS:FindFirstChild("Shared") and RS.Shared:FindFirstChild("Util") and require(RS.Shared.Util._t))
        or (RS:FindFirstChild("Util") and require(RS.Util._t))
        or (RS:FindFirstChild("Shared") and RS.Shared:FindFirstChild("Utils") and require(RS.Shared.Utils._t))
end)
local function _q3(_o13)
    local net = RS:FindFirstChild("Packages") and RS.Packages:FindFirstChild("Networking")
    return net and net:FindFirstChild(_o13)
end
local function _d4()
    if _j7 and _j7.ResolveLocalSlot then
        local ok, _a14 = pcall(_j7.ResolveLocalSlot)
        if ok and _a14 then return _a14 end
    end
    return 1
end
local function _y()
    local _x8 = _j7 and _j7.ResolvePlot and _j7.ResolvePlot()
    local pt = _x8 and _x8.CenterPoint and (typeof(_x8.CenterPoint) == "Vector3" and _x8.CenterPoint or (_x8.CenterPoint:IsA("BasePart") and _x8.CenterPoint.Position))
    if pt then
...
local _k1       = true
local Boss = { autoJoin = false, autoMastery = false, _u9 = {}, arenaReady = false }
local function _b4(_z6)
    local _g14 = _a9()
    if not _g14 or not _z6 then return false end
    _g14.CFrame = CFrame.new(_z6.X, math.max(_z6.Y, 70.0), _z6.Z)
    _g14.AssemblyLinearVelocity = Vector3.zero
    _g14.AssemblyAngularVelocity = Vector3.zero
    return true
end
local function _e2()
    local _v10 = Workspace:FindFirstChild("__DEBRIS")
    if not _v10 then return end
    for _, d in ipairs(_v10:GetChildren()) do
...
local function _y4(_h11, _b12, easeOut)
    local hrp = _a9()
    if not hrp or not _h11 then return false end
    local _y11 = hrp.Position
    local _p13 = (_h11 - _y11).Magnitude
    if _p13 < 1.0 then
        hrp.CFrame = CFrame.new(_h11.X, math.max(_h11.Y, 70.0), _h11.Z)
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        return true
    end
    _b12 = math.clamp(tonumber(_b12) or tonumber(_p5) or 750, 50, 750)
    local t0 = os.clock()
    local _t6 = _p13
    while not HUB.dead do
        local dt = RunService.Heartbeat:Wait()
        local _u10 = hrp.Position
        local _c8 = _h11 - _u10
        local _m11 = _c8.Magnitude
        if _m11 < 1.0 then break end
        local _s6 = _b12
        if easeOut then
            local _v7 = 1 - math.clamp(_m11 / _t6, 0, 1)
            _s6 = math.max(_b12 * (1 - _v7 * 0.8), 35)
        end
        local _i14 = math.min(_s6 * dt, _m11)
        local dir = _c8.Unit
        local _i9 = _u10 + dir * _i14
        hrp.CFrame = CFrame.lookAt(_i9, _i9 + dir)
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        if os.clock() - t0 > (_t6 / 50 + 5) then break end
    end
    hrp.CFrame = CFrame.new(_h11.X, math.max(_h11.Y, 70.0), _h11.Z)
    hrp.AssemblyLinearVelocity = Vector3.zero
    hrp.AssemblyAngularVelocity = Vector3.zero
...
local function _q5(_h11, _b12, easeOut)
    local hrp = _a9()
    if not hrp or not _h11 then return false end
    local _y11 = hrp.Position
    local _p13 = (_h11 - _y11).Magnitude
    ...
    _b12 = math.clamp(tonumber(_b12) or tonumber(_p5) or 750, 50, 750)
    local _o7 = math.max(_p13 / _b12, 0.02)
    if easeOut then
        _o7 = _o7 * 1.25
    end
    local t0 = os.clock()
    local _j12 = _h11 - _y11
    local dir = _j12.Magnitude > 0.001 and _j12.Unit or Vector3.new(1, 0, 0)
    while os.clock() - t0 < _o7 and not HUB.dead do
        local dt = RunService.Heartbeat:Wait()
        local _e5 = math.clamp((os.clock() - t0) / _o7, 0, 1)
        local a = _e5
        if easeOut then
            a = math.sin(_e5 * (math.pi / 2))
        end
        local cur = _y11:Lerp(_h11, a)
        hrp.CFrame = CFrame.lookAt(cur, cur + dir)
        local _x7 = _b12
        if easeOut then
            _x7 = math.max(_b12 * (1 - _e5 * 0.8), 35)
        end
        hrp.AssemblyLinearVelocity = Vector3.new(dir.X * _x7, math.clamp(dir.Y * _x7, -15, 150), dir.Z * _x7)
        hrp.AssemblyAngularVelocity = Vector3.zero
    end
    hrp.CFrame = CFrame.new(_h11.X, math.max(_h11.Y, 70.0), _h11.Z)
    hrp.AssemblyLinearVelocity = Vector3.zero
...
local _j2 = 580
local _g2 = 245
local function _o2(_z6, _b12, isApproach)
    local hrp = _a9()
    if not hrp or not _z6 then return false end
    if _k1 then pcall(_e2) end
    local _m8 = hrp.Position
    local _i12 = math.max(_m8.Y, _z6.Y, 70.4)
    local _i1 = (_z6.X < 560)
    if _i1 and _m8.X > _j2 then
        local p1 = Vector3.new(_m8.X, _i12, _r4)
        _y4(p1, _b12, false)
        local _k3 = Vector3.new(_j2, _i12, _r4)
        _y4(_k3, _b12, false)
        local _r6 = Vector3.new(_z6.X, _i12, _r4)
        _y4(_r6, _g2, false)
        local _d13 = _z6 + Vector3.new(0, 1.2, 0)
        _y4(_d13, _g2, isApproach == true)
        return true
    else
        local p1 = Vector3.new(_m8.X, _i12, _r4)
        local p2 = Vector3.new(_z6.X, _i12, _r4)
        local p3 = _z6 + Vector3.new(0, 1.2, 0)
        _y4(p1, _b12, false)
        _y4(p2, _b12, false)
        _y4(p3, _b12, isApproach == true)
        return true
    end
end
local function _h2(_z6, _b12, isApproach)
    local hrp = _a9()
    if not hrp or not _z6 then return false end
    if _k1 then pcall(_e2) end
    local _m8 = hrp.Position
    local _i1 = (_z6.X < 560)
    local _f5 = math.max(_m8.Y, _z6.Y, 70.4) + 28
    if _i1 and _m8.X > _j2 then
        local _s11 = Vector3.new(_m8.X, _f5, _m8.Z)
        local _t8 = Vector3.new(_j2, _f5, _r4)
        _q5(_s11, _b12, false)
        _q5(_t8, _b12, false)
        local _o4 = Vector3.new(_j2, 70.4, _r4)
        _q5(_o4, _g2, false)
        local _r6 = Vector3.new(_z6.X, 70.4, _r4)
        _y4(_r6, _g2, false)
        local _d13 = _z6 + Vector3.new(0, 1.2, 0)
        _y4(_d13, _g2, isApproach == true)
        return true
    else
        local _t6 = (_z6 - _m8).Magnitude
        if _t6 < 25 then
            _q5(Vector3.new(_z6.X, math.max(_z6.Y, 70.0) + 1.2, _z6.Z), _b12, isApproach == true)
            return true
        end
        local _s11 = Vector3.new(_m8.X, _f5, _m8.Z)
        local _k12 = Vector3.new(_z6.X, _f5, _z6.Z)
        local _a10 = Vector3.new(_z6.X, math.max(_z6.Y, 70.0) + 1.2, _z6.Z)
        _q5(_s11, _b12, false)
        _q5(_k12, _b12, false)
        _q5(_a10, _b12, isApproach == true)
        return true
    end
end
local function _r2(_z6)
    local hum = _e9()
    local hrp = _a9()
    if not hum or not hrp or not _z6 then return false end
    if _k1 then pcall(_e2) end
    local _m8 = hrp.Position
    local p1 = Vector3.new(_m8.X, _m8.Y, _r4)
    local p2 = Vector3.new(_z6.X, _z6.Y, _r4)
    local p3 = _z6 + Vector3.new(0, 1.2, 0)
...
local function _s(_z6, _b12, isApproach)
    if _o == "Deslize Voando" then
        return _h2(_z6, _b12, isApproach)
    elseif _o == "Caminhada Segura" then
        return _r2(_z6)
    else
        return _o2(_z6, _b12, isApproach)
    end
end
local _r1 = {
    ["LightDark"]       = 1300,
...
local _z = 0.1
local function _v(selected)
    if not selected or next(selected) == nil then return _i end
    return selected
end
local _w        = true
local _q2            = false
...
local _y3              = 0.2
local _b1        = true
local function _c2(egg)
    if not egg then return "Common", 100 end
    if egg.Rarity then
        local r = egg.Rarity
        local _o13 = type(r) == "table" and (r.DisplayName or r._id or r.Name) or tostring(r)
        local _y12 = _r1[_o13] or (type(r) == "table" and tonumber(r.RarityNumber) and r.RarityNumber * 100) or 100
        return _o13, _y12
    end
    local cat = egg.AssetCategory or egg.Category or egg.Name
    if cat and _y5 then
        local _a7 = _y5.Directory or _y5
        local _l12 = _a7[cat]
        if _l12 and _l12.Rarity then
            local r = _l12.Rarity
            local _o13 = type(r) == "table" and (r.DisplayName or r._id or r.Name) or tostring(r)
            local _y12 = _r1[_o13] or (type(r) == "table" and tonumber(r.RarityNumber) and r.RarityNumber * 100) or 100
            return _o13, _y12
    ...
    local _p8 = _e7 and (_e7.Directory or _e7) and (_e7.Directory or _e7)[egg.AreaId]
    local _r10 = _p8 and _p8.Rarity
    local _i8 = (type(_r10) == "table" and (_r10._id or _r10.DisplayName or _r10.Name)) or (type(_r10) == "string" and _r10) or "Common"
    local _o3 = _w5 and (_w5.Rarities or _w5) or {}
    local _p11 = _o3[_i8] or {}
    local _m1 = (type(_p11) == "table" and (_p11.DisplayName or _p11._id)) or (type(_r10) == "table" and _r10.DisplayName) or _i8 or "Common"
    local _v6 = _r1[_m1] or _r1[_i8] or (type(_r10) == "table" and tonumber(_r10.RarityNumber) and _r10.RarityNumber * 100) or 100
    return _m1, _v6
end
local function _i2(_g6, filter)
    if not filter or type(filter) ~= "table" then return true end
    local _r11 = 0
    for _ in pairs(filter) do _r11 = _r11 + 1 end
    if _r11 == 0 then return true end
    if filter[_g6] == true then return true end
    local _i11 = string._w12(tostring(_g6))
    for k, v in pairs(filter) do
        if type(v) == "string" and string._w12(v) == _i11 then
            return true
        elseif type(k) == "string" and string._w12(k) == _i11 and v == true then
            return true
        end
    end
    return false
end
local function _f3(_o13)
    local dir = _e7 and _e7.Directory
    if type(dir) ~= "table" then return tostring(_o13) end
    local _w12 = string._w12(tostring(_o13))
    for id, info in pairs(dir) do
        if string._w12(tostring(id)) == _w12 then return id end
        if type(info) == "table" and info.DisplayName
            and string._w12(tostring(info.DisplayName)) == _w12 then
            return id
        end
    end
    return tostring(_o13)
end
local function _h3(areaId, filter)
    if not filter or type(filter) ~= "table" then return true end
    local _r11 = 0
    for _ in pairs(filter) do _r11 = _r11 + 1 end
    if _r11 == 0 then return true end
    if filter[areaId] == true then return true end
    local _t10 = string._w12(tostring(areaId))
    for k, v in pairs(filter) do
        if type(v) == "string" and (string._w12(v) == _t10
            or string._w12(tostring(_f3(v))) == _t10) then
            return true
        elseif type(k) == "string" and string._w12(k) == _t10 and v == true then
            return true
        end
    end
    return false
end
local function _j1(_d14, _c11, filter)
    local _m5 = (_c11 and _c11.HasParasite == true)
        or (type(_d14) == "table" and (table.find(_d14, "Parasite") or table.find(_d14, "Monstruoso")))
        or (_c11 and (_c11.BaseMutation == "Parasite" or _c11.BaseMutation == "Monstruoso"))
    if not filter or type(filter) ~= "table" then return true end
    local _r11 = 0
    for _ in pairs(filter) do _r11 = _r11 + 1 end
    if _r11 == 0 then return true end
    local _a11 = type(_d14) == "table" and #_d14 > 0
    local _c10 = false
    for _, opt in pairs(filter) do
        if type(opt) == "string" then
            if opt == "Apenas Normal" and not _a11 and not _m5 then
                _c10 = true
            elseif opt == "Apenas Mutado" and (_a11 or _m5) then
                _c10 = true
            elseif (opt == "Parasita / Infestado" or opt == "Monstruoso") and _m5 then
                _c10 = true
            elseif opt == "Apenas Silver" and type(_d14) == "table" and table.find(_d14, "Silver") then
                _c10 = true
            elseif opt == "Apenas Gold" and type(_d14) == "table" and (table.find(_d14, "Gold") or table.find(_d14, "Golden")) then
                _c10 = true
            elseif opt == "Apenas Rainbow" and type(_d14) == "table" and table.find(_d14, "Rainbow") then
                _c10 = true
            end
        end
    end
...
local function _r8(_c11)
    if not _c11 then return false end
    local _q11 = tonumber(_c11.AssetScale) or 1
    local _f7 = tonumber(_c11.NestScale) or 1
    return _q11 >= 1.35 or _f7 >= 1.0
end
local function _k(areasFilter, raritiesFilter, mutationsFilter)
    if not _g8 or not _g8.ReadFieldEggs then return {} end
    local ok, _b8 = pcall(_g8.ReadFieldEggs)
    if not ok or not _b8 or not _b8.Records then return {} end
    local _l9 = {}
    for _, _c11 in ipairs(_b8.Records) do
        if _c11.State == "Slot" and _c11.BoundsCFrame then
            local _p6 = _j5[_c11.Uid] and (os.clock() - _j5[_c11.Uid] < 2.5)
            if not _p6 and (not _u1 or _r8(_c11)) then
                local _j10 = _h3(_c11.AreaId, areasFilter)
                local _g6, _v6 = _c2(_c11)
                local _y7 = _i2(_g6, raritiesFilter)
                local _d14 = _c11.Mutations or {}
                local _h12 = _j1(_d14, _c11, mutationsFilter)
                if _j10 and _y7 and _h12 then
                    local _r7 = 0
                    for _, m in ipairs(_d14) do
                        if m == "Rainbow" then _r7 = _r7 + 35
                        elseif m == "Gold" or m == "Golden" then _r7 = _r7 + 20
                        elseif m == "Silver" then _r7 = _r7 + 10 end
                    end
                    if _c11.HasParasite == true or (type(_d14) == "table" and (table.find(_d14, "Parasite") or table.find(_d14, "Monstruoso"))) then
                        _r7 = _r7 + 800
                    end
                    if _r8(_c11) then
                        _r7 = _r7 + 600
    ...
    if #_l9 > 1 then
        table.sort(_l9, function(a, b)
            return a._y12 > b._y12
        end)
    end
    return _l9
end
...
local function _q()
    ...
    local _p9 = pg and pg:FindFirstChild("DropHeldEgg")
    if _p9 and _p9.Enabled == true then
        return true
    end
    local _b13 = LP.Character
    if _b13 then
        for _, t in ipairs(_b13:GetChildren()) do
            if t:IsA("Model") and (t.Name:_w12():find("egg") or t:GetAttribute("Uid") or t:GetAttribute("AssetCategory")) then
                return true
            end
            if t:IsA("Tool") then
                if _p2 and _p2.IsEggTool and _p2.IsEggTool(t) then
                    return true
                end
                if t:GetAttribute("IsEgg") == true or t:GetAttribute("Uid") ~= nil or t:GetAttribute("AssetCategory") ~= nil then
                    return true
                end
                local _c12 = t.Name:_w12()
                if _c12:find("egg") or (_c12 ~= "bat" and _c12 ~= "defaulttool" and not _c12:find("bat") and not _c12:find("slap") and not _c12:find("coil") and not _c12:find("potion") and not _c12:find("lantern")) then
                    return true
                end
            end
        end
    ...
    if bp then
        for _, t in ipairs(bp:GetChildren()) do
            if t:IsA("Tool") and _p2 and _p2.IsEggTool and _p2.IsEggTool(t) then
                return true
            end
        end
    end
...
local function _b()
    local _x8 = _j7 and _j7.ResolvePlot and _j7.ResolvePlot()
    local _l5 = _x8 and _x8.CenterPoint and _x8.CenterPoint.Position or Vector3.new(464.7, 68.2, -364.0)
    local _c4 = {}
    for _, t in ipairs(LP.Character:GetChildren()) do
        if t:IsA("Tool") and _p2 and _p2.IsEggTool and _p2.IsEggTool(t) then
            local uid = _p2.GetToolUid(t)
            if uid then table.insert(_c4, uid) end
        end
    end
    ...
    local _t3 = 0
    for _, eggUid in ipairs(_c4) do
        for attempt = 1, 3 do
            local _f11 = CFrame.new(math.random(-6, 6), 0, math.random(-6, 6))
            local ok, res = pcall(function()
                if _g8 and _g8.PlantEgg then
                    return _g8.PlantEgg(eggUid, _f11)
                end
                return false
            end)
            if ok and res then
...
local function _f(targetItem)
    local _c11 = targetItem._c11 or targetItem
    if not _c11 or not _c11.Uid or not _c11.BoundsCFrame then return false end
    if _g8 and _g8.ReadFieldEggs then
        local ok, _m13 = pcall(_g8.ReadFieldEggs)
        if ok and _m13 and _m13.Records then
            local _f6 = false
            for _, r in ipairs(_m13.Records) do
                if r.Uid == _c11.Uid and r.State == "Slot" then
                    _f6 = true
                    _c11 = r
                    break
                end
    ...
    _a()
    local _z6 = _c11.BoundsCFrame.Position
    local _b12 = math.clamp(tonumber(_p5) or 750, 50, 750)
    _s(_z6 + Vector3.new(0, 1.2, 0), _b12, true)
    if hrp then
        hrp.CFrame = CFrame.new(_z6 + Vector3.new(0, 1.2, 0))
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
    end
    task.wait(0.5)
    local _t9 = nil
    if _t and _t.LooksLikeFirstAreaUid and _t.LooksLikeFirstAreaUid(_c11.Uid) then
        _t9 = _t.SlotKey(_c11.AreaId, _c11.NestId)
    end
    local net = RS:FindFirstChild("Packages") and RS.Packages:FindFirstChild("Networking")
    local _d5 = net and net:FindFirstChild("RF/EggWorld/AskFieldEggCarry")
    if _d5 then
        pcall(function() _d5:InvokeServer({ Uid = _c11.Uid, FirstAreaSlotKey = _t9 }) end)
    end
    pcall(function()
        if _g8 and _g8.CarryFieldEgg then
            _g8.CarryFieldEgg(_c11.Uid, _t9)
        end
    end)
    local _m10 = nil
    for _, d in ipairs(Workspace:GetDescendants()) do
        if d:IsA("ProximityPrompt") and d.Name == "CarryAreaEgg" and d.Enabled then
            local act = (d.ActionText or ""):_w12()
            local obj = (d.ObjectText or ""):_w12()
    ...
    if _m10 then
        _m10.HoldDuration = 0
        pcall(function() fireproximityprompt(_m10) end)
        pcall(function() fireproximityprompt(_m10, 0) end)
    end
    local _c3, _ = _y()
    local _a6 = CFrame.new(_c3 + Vector3.new(0, 1.2, 0))
    local _v9 = os.clock()
    local _f9 = false
    local _w9 = 1.5
    while os.clock() - _v9 < _w9 and not HUB.dead do
        if _f9 then
            break
        end
        if _q() then
    ...
    do
        ...
        if _f2 and _f9 then
            ...
            local _k4 = 100
            local _e13 = _e9()
            if _e13 then _k4 = _e13.Health end
            local _o10 = false
            while os.clock() - _k5 < 4.0 and not HUB.dead do
                if not _q() then
                    _o10 = true
                    break
                end
                ...
                if h then
                    local hs = h:GetState()
                    if h.Health < _k4 - 1.5 or hs == Enum.HumanoidStateType.Physics or hs == Enum.HumanoidStateType.Ragdoll or hs == Enum.HumanoidStateType.FallingDown then
                        _o10 = true
                        local _t11 = os.clock()
                        while os.clock() - _t11 < 0.85 and not HUB.dead do
                            if not _q() then break end
                            task.wait(0.05)
                        end
                        break
                    end
            ...
            if _o10 or not _q() then
                task.wait(0.65)
                do
                    local _f14 = os.clock()
                    while os.clock() - _f14 < 3.2 and not HUB.dead do
                        local h = _e9()
                ...
                local _e11 = _a9()
                if _e11 and (_e11.Position - _z6).Magnitude > 14 then
                    pcall(function()
                        _e11.CFrame = CFrame.new(_z6 + Vector3.new(0, 1.8, 0))
                        _e11.AssemblyLinearVelocity = Vector3.zero
                        _e11.AssemblyAngularVelocity = Vector3.zero
                    end)
                    task.wait(0.35)
                end
                do
                    local _g11 = os.clock()
                    while os.clock() - _g11 < 1.5 and not HUB.dead do
                        local h = _e9()
                ...
                do
                    local _h5 = os.clock()
                    while os.clock() - _h5 < 4.5 and not HUB.dead do
                        local _n4 = false
                        pcall(function()
                            local _y6 = Workspace:FindFirstChild("__OBJECTS") and Workspace.__OBJECTS:FindFirstChild("Areas") and Workspace.__OBJECTS.Areas:FindFirstChild("GuardAreas")
                            local _n5 = nil
                            if _y6 and _c11.AreaId then
                                local _v5 = _y6:FindFirstChild(_c11.AreaId)
                                if _v5 then
                                    _n5 = _v5:FindFirstChild("Guard") or _v5:FindFirstChild("ForestGuardAuthored") or _v5:FindFirstChildWhichIsA("Model", true)
                                end
                            end
                            if not _n5 then
                                local _w8, nd = nil, 1e9
                                for _, m in ipairs(Workspace:GetDescendants()) do
                                    if m:IsA("Model") and m.Name:_w12():find("guard") and m.PrimaryPart then
                                        local d = (m.PrimaryPart.Position - _z6).Magnitude
                                        if d < nd and d < 90 then nd = d; _w8 = m end
                                    end
                                end
                                _n5 = _w8
                            end
                            if _n5 then
                                local _s12 = _n5:GetAttribute("Alert") or _n5:GetAttribute("Alerted") or _n5:GetAttribute("IsAlerted") or _n5:GetAttribute("Chasing")
                                local _q7 = _n5:GetAttribute("Sleeping") or _n5:GetAttribute("IsSleeping") or _n5:GetAttribute("Asleep") or _n5:GetAttribute("Sleep")
                                local _w11 = _n5:GetAttribute("State")
                                if _q7 == true then _n4 = true
                                elseif _s12 == false or _s12 == nil then
                                    local hum = _n5:FindFirstChildOfClass("Humanoid")
                                    local hrp = _n5.PrimaryPart or _n5:FindFirstChild("HumanoidRootPart") or _n5:FindFirstChildWhichIsA("BasePart", true)
                                    local _k8 = _n5:FindFirstChild("EggPoint", true)
                                    if hrp and _k8 then
                                        local _s5 = (hrp.Position - _k8.Position).Magnitude
                                        if _s5 < 7 and (not hum or hum.MoveDirection.Magnitude < 0.12) then
                                            _n4 = true
                                        elseif _s5 < 12 and os.clock() - _h5 > 1.2 and (not hum or hum.MoveDirection.Magnitude < 0.15) then
                                            _n4 = true
                                        end
                                    elseif _w11 and tostring(_w11):_w12():find("sleep") then _n4 = true
                                    elseif _s12 == nil and _q7 == nil and _w11 == nil then
                                        if os.clock() - _h5 > 1.6 then _n4 = true end
                                    elseif _s12 == false then _n4 = true
                                    end
                                end
                                if not _n4 then
                                    local _n8 = _n5:FindFirstChild("Alert", true)
                                    if _n8 and _n8:IsA("BillboardGui") and _n8.Enabled == false then _n4 = true end
                                end
                            else
                                if os.clock() - _h5 > 1.4 then _n4 = true end
                            end
                        end)
                        if _n4 then break end
                        task.wait(0.14)
                    end
                    task.wait(0.08)
                end
                task.wait(0.08)
                pcall(function()
                    if _d5 then _d5:InvokeServer({ Uid = _c11.Uid, FirstAreaSlotKey = _t9 }) end
                end)
                pcall(function()
                    if _g8 and _g8.CarryFieldEgg then _g8.CarryFieldEgg(_c11.Uid, _t9) end
                end)
                task.wait(0.08)
                local _h10 = nil
                for _, d in ipairs(Workspace:GetDescendants()) do
                    if d:IsA("ProximityPrompt") and d.Name == "CarryAreaEgg" and d.Enabled then
                        ...
                        if p then
                            local _p13 = (p.Position - (_a9() and _a9().Position or _z6)).Magnitude
                            if _p13 < 16 then
                                local act = (d.ActionText or ""):_w12()
                ...
                if _h10 then
                    _h10.HoldDuration = 0
                    pcall(function() fireproximityprompt(_h10) end)
                    pcall(function() fireproximityprompt(_h10, 0) end)
                else
                    for _, d in ipairs(Workspace:GetDescendants()) do
                        if d:IsA("ProximityPrompt") and d.Name == "CarryAreaEgg" and d.Enabled then
                            local p = d.Parent
                            if p and p:IsA("Attachment") then p = p.Parent end
                            if p and (p.Position - (_a9() and _a9().Position or _z6)).Magnitude < 18 then
                                d.HoldDuration = 0
                                pcall(function() fireproximityprompt(d) end)
                                task.wait(0.08)
                                if _q() then break end
                            end
                        end
                    end
                end
                local _f8 = os.clock()
                while os.clock() - _f8 < 2.2 and not HUB.dead do
                    if _q() then _f9 = true break end
                    pcall(function()
                        if _g8 and _g8.CarryFieldEgg then _g8.CarryFieldEgg(_c11.Uid, _t9) end
                    end)
                    if _h10 then pcall(function() fireproximityprompt(_h10) end) end
                    task.wait(0.06)
                end
                if _q() then _f9 = true end
                if _q() then
                    task.wait(0.12)
                else
                    task.wait(0.12)
    ...
    if _b12 > 250 then
        local _e11 = _a9()
        local _u10 = _e11 and _e11.Position or _z6
        local _l10 = _c3 - _u10
        local _t7 = _l10.Magnitude
        if _t7 > 45 then
            local _z7 = _c3 - _l10.Unit * 35
            _z7 = Vector3.new(_z7.X, math.max(_z7.Y, 70.4), _z7.Z)
            _s(_z7, _b12, false)
            local _q13 = _a9()
            if _q13 then
                _q13.AssemblyLinearVelocity = Vector3.zero
                _q13.AssemblyAngularVelocity = Vector3.zero
            end
            task.wait(0.35)
        end
        _s(_c3, 240, true)
    ...
    local _u7 = os.clock()
    while os.clock() - _u7 < 1.5 and _q() and not HUB.dead do
        task.wait(0.08)
    end
    _b()
    local _b13 = LP.Character
    local h = _b13 and _b13:FindFirstChild("HumanoidRootPart")
    local hu = _b13 and _b13:FindFirstChildOfClass("Humanoid")
    if h then
        h.CFrame = CFrame.new(_c3.X, math.max(_c3.Y, 70.4), _c3.Z)
...
local function _h1()
    if not _g8 or not _g8.ReadOwnedEggs then return 0 end
    local ok, _b8 = pcall(_g8.ReadOwnedEggs, LP.UserId)
    if not ok or not _b8 then return 0 end
    local _r11 = 0
    local _b10 = _b8.Records or _b8
    if typeof(_b10) == "table" then
        for uid, eggData in pairs(_b10) do
            if typeof(eggData) == "table" then
                local _d10 = false
                if _g8.IsReadyToHatch then
                    _d10 = _g8.IsReadyToHatch(eggData)
                else
                    _d10 = eggData.Placement ~= nil
                end
                if _d10 then
                    pcall(function()
                        if _g8.BeginHatch then _g8.BeginHatch(uid) end
                        task.wait(0.05)
                        if _g8.FinishHatch then _g8.FinishHatch(uid) end
                        _r11 = _r11 + 1
                    end)
                end
            end
...
local function _l()
    local re1 = _q3("RE/Homestead/AskNearbyPurchase")
    if re1 then pcall(function() re1:FireServer() end) end
    local re2 = _q3("RE/Homestead/AskBaseTierRaise")
    if re2 then pcall(function() re2:FireServer() end) end
end
local function _m()
    local rf = _q3("RF/Treadmill/AskTierRaise")
    if rf then pcall(function() rf:InvokeServer() end) end
end
local function _g3()
    local rf = _q3("RF/Haul/WearBest") or _q3("RF/PenRoster/ConfirmEquipBestBadge")
    if rf then pcall(function() rf:InvokeServer() end) end
end
Boss.Data = nil
...
function Boss.Snapshot()
    local rf = _q3("RF/BossEvent/AskSnapshot")
...
function Boss.IsOpen()
    Boss.EnsureData()
    local _m13 = Boss.Snapshot()
    if _m13 then
        if _m13.Open ~= nil then return _m13.Open == true end
        if _m13.BossHealth and _m13.BossMaxHealth then
            return (tonumber(_m13.BossHealth) or 0) > 0
        end
    end
    if Boss.Data and type(Boss.Data.SecondsUntilNextOpen) == "function" then
        local ok, _u13 = pcall(function() return Boss.Data.SecondsUntilNextOpen() end)
        if ok and tonumber(_u13) then return tonumber(_u13) <= 0 end
    end
    return false
end
function Boss.SecondsUntilOpen()
    ...
    if Boss.Data and type(Boss.Data.SecondsUntilNextOpen) == "function" then
        local ok, _u13 = pcall(function() return Boss.Data.SecondsUntilNextOpen() end)
        if ok and tonumber(_u13) then return tonumber(_u13) end
    end
    return nil
end
function Boss.Join()
    local rf = _q3("RF/BossEvent/AskEnter")
...
function Boss.ClaimMastery()
    Boss.EnsureData()
    local rf = _q3("RF/BossMastery/AskClaimMilestone")
    ...
    if Boss.MasteryData and type(Boss.MasteryData.Milestones) == "table" then
        for _, m in pairs(Boss.MasteryData.Milestones) do
            if type(m) == "table" and type(m.Id) == "string" and not Boss._u9[m.Id] then
    ...
    if #ids == 0 then
        for _, id in ipairs(Boss.MilestoneFallback) do
            if not Boss._u9[id] then table.insert(ids, id) end
        end
    end
    local _u9 = 0
...
function Boss.FindBat()
    local _b13 = LP.Character
    if not _b13 then return nil end
    local _x13 = _b13:FindFirstChildWhichIsA("Tool")
    if _x13 and _x13:GetAttribute("IsBat") == true then return _x13 end
    ...
    local _y13 = _q3("RF/Codex/AskWearFieldBat")
    if _y13 then pcall(function() _y13:InvokeServer() end) end
    task.wait(0.25)
    if bag then
        for _, c in ipairs(bag:GetChildren()) do
            if c:IsA("Tool") and c:GetAttribute("IsBat") == true then
...
function Boss.FindTarget()
    local _x12 = Workspace:FindFirstChild("BossArena")
    if not _x12 then return nil end
    local _g14 = _a9()
    if not _g14 then return nil end
    local _l13, _a8 = nil, math.huge
    local _b11 = _x12:FindFirstChild("CrystalTowers")
    if _b11 then
        for _, tower in ipairs(_b11:GetChildren()) do
            local hb = tower:FindFirstChild("Hitbox", true)
            if hb and hb:IsA("BasePart") then
                local hp = tonumber(hb:GetAttribute("Health"))
                if hp == nil or hp > 0 then
                    local d = (_g14.Position - hb.Position).Magnitude
                    if d < _a8 then _l13, _a8 = hb, d end
                end
            end
        end
    end
    if _l13 and Boss.arenaApproach == "Cristais Primeiro" then
        return _l13, "Crystal"
    end
    local _h13 = _x12:FindFirstChild("Boss")
    if _h13 then
        local aim = _h13:FindFirstChild("UpperHand1.R", true) or _h13.PrimaryPart
        if aim and aim:IsA("BasePart") then
            local d = (_g14.Position - aim.Position).Magnitude
            if d < _a8 then _l13, _a8 = aim, d end
        end
    end
    return _l13, (_l13 and _l13:IsDescendantOf(_b11 or _x12) and "Boss" or nil)
end
function Boss.GlideStep(_h11)
    local _g14 = _a9()
    if not _g14 or not _h11 then return false end
    local _f11 = _g14.Position - _h11.Position
    _f11 = Vector3.new(_f11.X, 0, _f11.Z)
    if _f11.Magnitude < 0.5 then _f11 = Vector3.new(0, 0, 1) end
    local _c5 = _h11.Position + _f11.Unit * 5
    local _k13 = _c5 - _g14.Position
    local _m11 = _k13.Magnitude
    if _m11 < 1.0 then
        _g14.AssemblyLinearVelocity = Vector3.zero
        _g14.AssemblyAngularVelocity = Vector3.zero
        return true
    ...
    Boss._stepAt = now
    local _b12 = math.clamp(tonumber(Boss._p5) or 260, 60, 500)
    local dir = _k13.Unit
    local _i14 = math.min(_b12 * dt, _m11)
    local _i9 = _g14.Position + dir * _i14
    local _g13 = Vector3.new(dir.X, 0, dir.Z)
    if _g13.Magnitude < 0.01 then _g13 = _g14.CFrame.LookVector end
    _g14.CFrame = CFrame.lookAt(_i9, _i9 + _g13.Unit)
    _g14.AssemblyLinearVelocity = Vector3.zero
    _g14.AssemblyAngularVelocity = Vector3.zero
    return false
end
...
function Boss.CurrentTarget()
    local now = os.clock()
    local _x13 = Boss._targetPart
    if _x13 and _x13.Parent and (now - (Boss._targetAt or 0)) < 0.35 then
        local hp = tonumber(_x13:GetAttribute("Health"))
        if hp == nil or hp > 0 then return _x13, Boss._target end
    end
    local _c13, _w13 = Boss.FindTarget()
    Boss._targetPart, Boss._target, Boss._targetAt = _c13, _w13, now
    return _c13, _w13
end
function Boss.Fight()
    if not Boss.IsInArena() then return false end
    local _g14 = _a9()
    if not _g14 then return false end
    local _h11, _w13 = Boss.CurrentTarget()
    if not _h11 then return false end
    local _p13 = (_g14.Position - _h11.Position).Magnitude
    ...
    local _u11 = _q3("RE/BatSwing/Trigger")
    if _u11 then pcall(function() _u11:FireServer() end) end
    return true
end
Boss._hazardRemotes = {}
...
do
    local _n11 = _q3("RE/BossEvent/HazardHit")
    local _k6 = _q3("RE/BossEvent/BlackHoleHit")
    for _, remote in ipairs({ _n11, _k6 }) do
...
function Boss.InstallHazardHook()
    ...
    local _g7 = false
    pcall(function()
        _g7 = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
    end)
    if _g7 then
        Notify("Perigos do Chefe", "Imunidade a perigos não suportada no mobile - o chefe ainda pode te acertar", "Error")
        return false
    end
    if not _l11 then return false end
    local _n11 = _q3("RE/BossEvent/HazardHit")
    if type(_n11) ~= "userdata" or not _n11:IsA("RemoteEvent") then return false end
    local _q9 = _n11.FireServer
    if type(_q9) ~= "function" then return false end
    local ok = pcall(function()
        _l11(_q9, function(self, ...)
...
local function _p4()
    local rf = _q3("RF/EggWorld/AskFieldEggDrop")
    if rf then pcall(function() rf:InvokeServer() end) end
    if _g8 and _g8.DropFieldEgg then pcall(_g8.DropFieldEgg) end
end
local function _p()
    local rf = _q3("RF/Trailwear/AskPurchase")
    local _t5 = RS:FindFirstChild("Data") and RS.Data:FindFirstChild("Trails") and require(RS.Data.Trails)
    local _s13 = nil
    pcall(function() _s13 = _o5 and _o5.Get and _o5.Get() end)
    if not rf or not _t5 or not _s13 then return end
    local _g9 = tonumber(_s13.Money) or 0
    local inv = _s13.TrailInventory or {}
    for _, t in pairs(_t5.Directory or _t5) do
        if type(t) == "table" and t._id and not inv[t._id] then
            local _p12 = tonumber(t.Price) or math.huge
            if _g9 >= _p12 then
...
local function _b3(enabled)
    _w = enabled
    if enabled then
        pcall(function()
            local _z8 = _q3("RE/RigSync/Refresh")
            if _z8 and getconnections then
                for _, conn in ipairs(getconnections(_z8.OnClientEvent)) do
                    pcall(function() conn:Disconnect() end)
                end
            end
        end)
...
pcall(function() if _k1 then _e2() end end)
pcall(function() if _w then _b3(true) end end)
local function _q1()
    local re = _q3("RE/PetSatchel/SellPet")
    if not re or not _o5 then return end
    local _s13 = nil
    pcall(function() _s13 = _o5.Get and _o5.Get() end)
    local inv = _s13 and _s13.Inventory
    if type(inv) ~= "table" then return end
    for uid, petData in pairs(inv) do
        if type(petData) == "table" and not petData.Locked then
            local _m12 = petData.Rarity or "Common"
            if _i2(_m12, _v(_e)) then
...
local function _a2()
    if not _o5 then return end
    local _s13 = nil
    pcall(function() _s13 = _o5.Get and _o5.Get() end)
    if not _s13 then return end
    local inv = _s13.EggInventory
    if type(inv) ~= "table" then return end
    local _y13 = _q3("RF/EggWorld/AskWearTool")
    local _t13 = _q3("RE/PetSatchel/SellPet")
    if not _y13 or not _t13 then return end
    for uid, eggData in pairs(inv) do
        if type(eggData) == "table" and not eggData.Placement and not eggData.Locked then
            local _m12 = _c2(eggData)
            if _i2(_m12, _v(_d)) then
                pcall(function() _y13:InvokeServer(uid) end)
                pcall(function() _t13:FireServer({ uid }) end)
                task.wait(_z)
            end
        end
    end
...
local function _u()
    ...
    _z11(Workspace:FindFirstChild("Pets"))
    _z11(Workspace:FindFirstChild("RenderedPets"))
    return _r11
end
local function _c()
    pcall(function()
        local rf1 = _q3("RF/AwayEarnings/AskCollect")
        if rf1 then rf1:InvokeServer() end
    end)
    pcall(function()
        local rf2 = _q3("RF/Codex/AskRedeemAll")
        if rf2 then rf2:InvokeServer() end
    end)
    pcall(function()
        local rf3 = _q3("RF/GroupPerk/RedeemPerk")
...
task.spawn(function()
    while not HUB.dead do
        if _m2 then pcall(_l) end
        if _n then pcall(_m) end
        if _n1 then pcall(_g3) end
        if _w1 then pcall(_c) end
        if Boss.autoMastery then pcall(Boss.ClaimMastery) end
        if _z3 then pcall(_q1) end
        if _a4 then pcall(_a2) end
        task.wait(2.5)
    end
end)
task.spawn(function()
    while not HUB.dead do
        if Boss.autoJoin or Boss.autoFight then
            if Boss.IsInArena() then
                ...
                local ok, _c14 = pcall(Boss.IsOpen)
                Boss.arenaReady = (ok and _c14 == true)
...
task.spawn(function()
    local _u12 = _q3("RE/BatSwing/Trigger")
    while not HUB.dead do
        if _q2 and _u12 then
            local hrp = _a9()
            if hrp then
                local _a5 = false
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LP and p.Character then
                        local _i13 = p.Character:FindFirstChild("HumanoidRootPart")
                        if _i13 and (_i13.Position - hrp.Position).Magnitude <= _n3 then
                            _a5 = true
                            break
                        end
                    end
                ...
                if _a5 then
                    pcall(function() _u12:FireServer() end)
                end
            end
        end
        task.wait(_y3)
...
task.spawn(function()
    local _v10 = Workspace:FindFirstChild("__DEBRIS")
    if _v10 then
        _a12(_v10.ChildAdded:Connect(function(child)
...
local _c6 = type(Drawing) == "table" and type(Drawing.new) == "function"
local _l1 = {}
local _i3 = {}
local _s3 = nil
local function _k2()
    if _s3 and _s3.Parent then return _s3 end
    local p = nil
    ...
    _s3 = Instance.new("Folder")
    _s3.Name = "SAE_Esp_Holder"
    pcall(function() _s3.Parent = p end)
    return _s3
end
local function _d1(key, pos, icon)
    local bb = _i3[key]
    if not bb or not bb.gui or not bb.gui.Parent then
        local _o11 = _k2()
        local _c13 = Instance.new("Part")
        _c13.Name = "EspAnchor"
        _c13.Size = Vector3.new(1, 1, 1)
        _c13.Transparency = 1
        _c13.Anchored = true
        _c13.CanCollide = false
        _c13.CanQuery = false
        _c13.CanTouch = false
        _c13.CFrame = CFrame.new(pos)
        _c13.Parent = _o11
...
local function _r()
    if not _c6 then return {} end
    local o = {}
    o._o13 = _v3(Drawing.new("Text"))
    o._o13.Size = 13; o._o13.Center = true; o._o13.Outline = true; o._o13.Visible = false
    o._p13 = _v3(Drawing.new("Text"))
    o._p13.Size = 11; o._p13.Center = true; o._p13.Outline = true; o._p13.Visible = false
    o.box = _v3(Drawing.new("Square"))
    o.box.Thickness = 1.5; o.box.Filled = false; o.box.Visible = false
    return o
end
_a12(RunService.RenderStepped:Connect(function()
    if HUB.dead or not esp.enabled then
        for _, obj in pairs(_l1) do
            if obj._o13 then obj._o13.Visible = false end
            if obj._p13 then obj._p13.Visible = false end
            if obj.box then obj.box.Visible = false end
        end
        for _, bb in pairs(_i3) do
            if bb.gui then bb.gui.Enabled = false end
        end
        return
    end
    local hrp = _a9()
    local _n12 = hrp and hrp.Position or Vector3.zero
    local _g5 = {}
    local _i4 = {}
    if esp._h14 and _g8 and _g8.ReadFieldEggs then
        local ok, _m13 = pcall(_g8.ReadFieldEggs)
        if ok and _m13 and _m13.Records then
            for _, egg in ipairs(_m13.Records) do
                if egg.State == "Slot" and egg.BoundsCFrame then
                    local pos = egg.BoundsCFrame.Position
                    local _p13 = (pos - _n12).Magnitude
                    if esp.maxDistance <= 0 or _p13 <= esp.maxDistance then
                        local _d14 = egg.Mutations or {}
                        local _s10 = #_d14 > 0
                        if not esp.rareEggsOnly or _s10 then
                            local _x9 = _s10 and (" [" .. table.concat(_d14, ",") .. "]") or ""
                            local _m12 = _c2(egg)
                            local _v11 = (egg.AssetCategory or "Egg") .. " (" .. _m12 .. ")" .. _x9
                            local cat = egg.AssetCategory
                            local _l12 = _y5 and (_y5.Directory or _y5) and (_y5.Directory or _y5)[cat]
                            local _j9 = _l12 and (_l12.Icon or (_l12.Egg and _l12.Egg.Icon)) or ""
                            local _q6 = _s10 and esp.rareEggColor or esp.eggColor
                            table.insert(_g5, {
                                Key = egg.Uid,
                                Pos = pos,
                                Name = _v11,
    ...
    if esp.traps then
        local _v10 = Workspace:FindFirstChild("__DEBRIS")
        if _v10 then
            for _, trap in ipairs(_v10:GetChildren()) do
                if trap.Name == "PlayerTrap" and trap:IsA("BasePart") then
                    local pos = trap.Position
                    local _p13 = (pos - _n12).Magnitude
                    if esp.maxDistance <= 0 or _p13 <= esp.maxDistance then
                        local _f12 = trap:GetAttribute("Owner") or "Enemy"
                        table.insert(_g5, {
                            Key = trap,
    ...
    if esp.players then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP and p.Character then
                local _i13 = p.Character:FindFirstChild("HumanoidRootPart")
                if _i13 then
                    local _p13 = (_i13.Position - _n12).Magnitude
                    if esp.maxDistance <= 0 or _p13 <= esp.maxDistance then
                        table.insert(_g5, {
                            Key = p,
                            Pos = _i13.Position,
    ...
    for k, bb in pairs(_i3) do
        if not _i4[k] and bb.gui then
            bb.gui.Enabled = false
        end
    end
    local cam = _u6()
    local _e6 = {}
    for _, item in ipairs(_g5) do
        _e6[item.Key] = true
        local obj = _l1[item.Key]
        if not obj then
            obj = _r()
            _l1[item.Key] = obj
        end
        local _n7, _d8 = nil, false
        if cam then
            _n7, _d8 = cam:WorldToViewportPoint(item.Pos)
        end
        if _d8 and _c6 and _n7 then
            if obj._o13 then
                obj._o13.Text = item.Name
                obj._o13.Position = Vector2.new(_n7.X, _n7.Y - 14)
                obj._o13.Color = item.Color
                obj._o13.Visible = true
            end
            if obj._p13 then
                obj._p13.Text = math.floor(item.Dist) .. " studs"
                obj._p13.Position = Vector2.new(_n7.X, _n7.Y + 2)
                obj._p13.Color = Color3.fromRGB(220, 220, 220)
                obj._p13.Visible = true
            end
        else
            if obj._o13 then obj._o13.Visible = false end
            if obj._p13 then obj._p13.Visible = false end
...
local _g1 = false
local _v2 = Lighting.Ambient
local _u2 = Lighting.OutdoorAmbient
local _e1 = Lighting.Brightness
local _t1 = Lighting.ClockTime
local function _l3(v)
    _g1 = v
    if v then
        ...
        Lighting.Ambient = _v2
        Lighting.OutdoorAmbient = _u2
        Lighting.Brightness = _e1
        Lighting.ClockTime = _t1
    end
end
local _s1 = false
local _h4     = 24
...
local _s7         = 60
local _o9          = false
local function _z2(v)
    _h4 = v
    local hum = _e9()
    if hum and _s1 then hum.WalkSpeed = v end
end
local function _t2(v)
    _f4 = v
    local hum = _e9()
    if hum and _z1 then
...
_a12(RunService.Stepped:Connect(function()
    if HUB.dead then return end
    local hum = _e9()
    if hum then
        if _s1 then hum.WalkSpeed = _h4 end
        if _z1 then hum.UseJumpPower = true; hum.JumpPower = _f4 end
    end
end))
_a12(UserInputService.JumpRequest:Connect(function()
    if HUB.dead then return end
    local hum = _e9()
    if hum then
        hum.Jump = true
...
local function _u8()
    if _y10 then return end
    local hrp = _a9()
    local hum = _e9()
    if not (hrp and hum) then return end
    _y10 = true
    hrp.Anchored = true
    local _w7 = Instance.new("BodyGyro")
    _w7.MaxTorque = Vector3.new(1, 1, 1) * 1e5
    _w7.P = 1e5
    _w7.CFrame = hrp.CFrame
    _w7.Parent = hrp
    HUB._fly = {
        hrp = hrp,
        gyro = _w7,
        conn = _a12(RunService.RenderStepped:Connect(function(dt)
            if not _y10 or HUB.dead then return end
            local cam = _u6()
            if not cam then return end
            local _j13 = cam.CFrame.LookVector
            local _t12 = cam.CFrame.RightVector
            local _p7 = Vector3.new(_j13.X, 0, _j13.Z)
            _p7 = _p7.Magnitude > 0.001 and _p7.Unit or Vector3.new(0, 0, -1)
            local _m7 = Vector3.new(_t12.X, 0, _t12.Z)
            _m7 = _m7.Magnitude > 0.001 and _m7.Unit or Vector3.new(1, 0, 0)
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.A) then dir = dir - _m7 end
            if UserInputService:IsKeyDown(Enum.KeyCode.D) then dir = dir + _m7 end
            if UserInputService:IsKeyDown(Enum.KeyCode.Space) then dir = dir + Vector3.new(0, 1, 0) end
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) then dir = dir - Vector3.new(0, 1, 0) end
            if dir.Magnitude > 0 then
                hrp.CFrame = hrp.CFrame + dir.Unit * _s7 * math.min(dt, 0.1)
            end
            _w7.CFrame = CFrame.lookAt(hrp.Position, hrp.Position + _j13)
        end))
    }
end
local function _h9()
...
local _m4 = nil
local function _z5(v)
    _o9 = v
    if v and not _m4 then
        _m4 = _a12(LocalPlayer.Idled:Connect(function()
            if _o9 then
                _q4:CaptureController()
                _q4:ClickButton2(Vector2.new())
            end
        end))
    elseif not v and _m4 then
        pcall(function() _m4:Disconnect() end)
        _m4 = nil
    end
end
local _g10     = Window:AddTab({ Name = "Ovos", Subtitle = "Roubar, chocar e plantar", Icon = "crown" })
local _k9     = Window:AddTab({ Name = "Base", Subtitle = "Fazenda e treino", Icon = "bolt" })
local _c7   = Window:AddTab({ Name = "Combate", Subtitle = "Bastão, tapas e defesa", Icon = "combat" })
local _o6   = Window:AddTab({ Name = "Jogador", Subtitle = "Movimento e teleporte", Icon = "player" })
local _x4 = Window:AddTab({ Name = "Configurações", Subtitle = "Configs e descarregar", Icon = "gear" })
local _s8 = _g10:AddSubTab("Roubo Automático")
local _h8 = _g10:AddSubTab("Chocar e Plantar")
local _h7 = _g10:AddSubTab("ESP de Ovos")
_s8:AddToggle({
    Name = "Roubar Ovos Automaticamente", Default = false, Flag = "steal_auto",
    Callback = _w3(function(v)
        _x1 = v
        if v then _a() end
...
_s8:AddToggle({
    ...
    Callback = function(v) _p3 = v end
})
_s8:AddMultiDropdown({
    Name = "Filtrar por Raridade (Multi-seleção)", Options = _g4, Default = {}, Flag = "steal_rarities",
    Callback = function(selectedList) _h = selectedList end
})
_s8:AddMultiDropdown({
    Name = "Filtrar por Área (Multi-seleção)", Options = _r5, Default = {}, Flag = "steal_areas",
    Callback = function(selectedList) _a1 = selectedList end
})
_s8:AddMultiDropdown({
    Name = "Filtrar por Mutação (Multi-seleção)", Options = _p1, Default = {}, Flag = "steal_muts",
    Callback = function(selectedList) _j = selectedList end
})
_s8:AddSlider({
    Name = "Velocidade de Deslize / Viagem", Min = 50, Max = 750, Default = 750, Suffix = " studs/s", Flag = "glide_speed",
    Callback = function(v) _p5 = tonumber(v) or 750 end
})
_s8:AddSlider({
...
_h8:AddSlider({
    ...
    Callback = function(v) _l2 = v end
})
_h8:AddButton({
    Name = "Chocar Todos os Ovos Prontos Agora", Primary = true,
    Callback = _w3(function()
        local _r11 = _h1()
        Notify("Chocar", "Chocou " .. _r11 .. " ovo(s)", "Success")
    end)
})
_h8:AddButton({
    ...
    Callback = _w3(function()
        local _r11 = _b()
        Notify("Plantar Ovos", "Plantou " .. _r11 .. " ovo(s) no curral", "Success")
    end)
})
_h7:AddToggle({
...
do
local _z4 = _k9:AddSubTab("Fazenda e Esteira")
local _s9     = _k9:AddSubTab("Pets e Bolsa")
local _e8    = _k9:AddSubTab("Venda Automática")
local _i7   = _k9:AddSubTab("Eventos e Chefes")
local _x5  = _k9:AddSubTab("Resgatar Recompensas")
_z4:AddToggle({
    Name = "Melhorar Base / Plot Automaticamente", Default = false, Flag = "up_base_auto",
    Callback = function(v) _m2 = v end
})
_z4:AddToggle({
    Name = "Melhorar Nível da Esteira Automaticamente", Default = false, Flag = "up_tread_auto",
    Callback = function(v) _n = v end
})
_z4:AddToggle({
    Name = "Comprar Trilhas de Velocidade Automaticamente", Default = false, Flag = "auto_buy_trails",
    Callback = function(v) _d3 = v end
})
_z4:AddButton({
...
_e8:AddToggle({
    ...
    Callback = function(v) _z3 = v end
})
_e8:AddMultiDropdown({
    Name = "Filtrar Raridades de Pets para Venda", Options = _g4, Default = {}, Flag = "sell_pet_rarities",
...
_e8:AddMultiDropdown({
    Name = "Filtrar Raridades de Ovos para Venda", Options = _g4, Default = {}, Flag = "sell_egg_rarities",
...
_i7:AddButton({
    ...
    Callback = _w3(function()
        local _m13 = Boss.Snapshot()
        if _m13 and _m13.Open then
            local hp = tonumber(_m13.BossHealth) or 0
            local _r12 = tonumber(_m13.BossMaxHealth) or 0
            Notify("Arena do Chefe", "ABERTA - " .. tostring(math.floor(hp)) .. "/" .. tostring(math.floor(_r12)) .. " HP", "Success")
        else
            local _u13 = Boss.SecondsUntilOpen()
            local eta = "unknown"
            if _u13 then eta = string.format("%d min %d s", math.floor(_u13 / 60), math.floor(_u13 % 60)) end
...
do
local _x10   = _c7:AddSubTab("Aura de Bastão")
local _v8 = _c7:AddSubTab("Defesa e Guardas")
_x10:AddToggle({
...
_v8:AddToggle({
    ...
    Callback = function(v) _b1 = v end
})
_a12(RunService.Heartbeat:Connect(function()
    if HUB.dead or not _b1 then return end
    local hum = _e9()
...
do
local _m9     = _o6:AddSubTab("Movimento")
local _l7   = _o6:AddSubTab("Viajar para Área")
local _m6   = _o6:AddSubTab("Viajar para Plot")
local _l4 = _o6:AddSubTab("Viajar até Jogador")
local _y9     = _o6:AddSubTab("Visuais e Desempenho")
_m9:AddToggle({
...
_m9:AddSlider({
    ...
    Callback = function(v) _s7 = v end
})
_m9:AddToggle({
    Name = "Anti-AFK (Evitar Kick de 20 min)", Default = false, Flag = "anti_afk",
    Callback = function(v) _z5(v) end
})
local _n2 = "Base / Plot"
local _j8 = {}
for k in pairs(_v1) do table.insert(_j8, k) end
table.sort(_j8)
_l7:AddDropdown({
    Name = "Selecionar Área", Options = _j8, Items = _j8, Default = "Base / Plot", Flag = "tele_area",
    Callback = function(v) _n2 = v end
})
_l7:AddButton({
    Name = "Viajar para Área Selecionada", Primary = true,
    Callback = _w3(function()
        local pos = _v1[_n2]
        if _n2 == "Base / Plot" then
            pos = _y()
        end
        if pos then
            Notify("Viagem", "Viajando para " .. _n2, "Info")
            _o2(pos, _p5 or 200)
            Notify("Viagem", "Chegou em " .. _n2, "Success")
        else
...
local _d2 = "Plot 1"
local _b5 = { "Plot 1", "Plot 2", "Plot 3", "Plot 4", "Plot 5", "Plot 6", "Plot 7", "Meu Plot" }
_m6:AddDropdown({
    Name = "Selecionar Plot", Options = _b5, Items = _b5, Default = "Meu Plot", Flag = "tele_plot",
    Callback = function(v) _d2 = v end
})
_m6:AddButton({
    Name = "Viajar para o Plot", Primary = true,
    Callback = _w3(function()
        local _z9 = _d2 == "Meu Plot" and _d4() or tonumber(_d2:match("%d+")) or 1
        local _e14 = Workspace.Plots:FindFirstChild(tostring(_z9))
        local _z6 = _e14 and (_e14:FindFirstChild("CenterPoint") and _e14.CenterPoint.Position or _e14:GetPivot().Position)
        if _z6 then
            _o2(_z6 + Vector3.new(0, 2, 0), _p5 or 200)
            Notify("Plot", "Chegou no Plot " .. tostring(_z9), "Success")
        else
...
local function _e3()
    local _q12 = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP then table.insert(_q12, p.Name) end
    end
    table.sort(_q12)
    if #_q12 == 0 then _q12 = { "(nenhum outro jogador)" } end
    return _q12
end
local _y2 = _l4:AddDropdown({
    Name = "Selecionar Jogador", Options = _e3(), Items = _e3(), Default = nil, Flag = "tele_plr",
    Callback = function(v) _x = v end
})
_l4:AddButton({
...
_l4:AddButton({
    ...
    Callback = _w3(function()
        ...
        local _d7 = Players:FindFirstChild(_x)
        local _r13 = _d7 and _d7.Character and _d7.Character:FindFirstChild("HumanoidRootPart")
        if _r13 then
            _o2(_r13.Position + Vector3.new(0, 2, 0), _p5 or 200)
            Notify("Jogador", "Chegou em " .. _x, "Success")
        else
...
_y9:AddButton({
    ...
    Callback = _w3(function()
        local _r11 = _u()
        Notify("Desempenho", "Removeu " .. _r11 .. " modelo(s) de pet", "Success")
    end)
})
end
do
local _w6 = _x4:AddSubTab("Configuração")
if _h6 then
    _w6:AddInput({
        Name = "Nome da Config", Default = _t4, Flag = "cfg_name",
        Callback = function(v) if v and #v > 0 then _t4 = v end end
    })
    _w6:AddButton({
        Name = "Salvar Config", Primary = true,
        Callback = _w3(function()
            local ok, err = Library:SaveConfig(_t4)
            Notify("Config", ok and ("Config '" .. _t4 .. "' salva") or ("Falha ao salvar: " .. tostring(err)), ok and "Success" or "Error")
        end)
    })
    _w6:AddButton({
        Name = "Carregar Config",
        Callback = _w3(function()
            local ok, err = Library:LoadConfig(_t4)
            if ok then
                _k7()
                Notify("Config", "Config '" .. _t4 .. "' carregada", "Success")
            else
